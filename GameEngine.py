    ...
    for y in range(max(0, int(start[1])), min(SCREEN_H, int(stop[1]+1))):
        ...
        if xx1 != xx2:
            ...
            for x in range(xx1, xx2):
                delta_x = x - x1
                z = 1/(z1 + delta_x*z_slope + 1e-32)
                if z < z_buffer[x][y]:
                    u = (u1 + delta_x*u_slope)*z
                    v = (v1 + delta_x*v_slope)*z
                    if min(u, v) >= 0 and max(u, v) <= 1:
                        z_buffer[x][y] = z
...
@njit()
def filter_triangles(z_min, normal, CameraRay, xxs, yys):
